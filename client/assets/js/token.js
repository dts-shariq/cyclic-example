...
Parse.initialize(("021"));  // Application id from moralis.io
Parse.serverURL = 'http://192.168.100.161:1337/server'; //Server url from moralis.io
...
$('#connectWalletModalBtn').click(async () =>{
  ...
  $('#connectWalletBtn').html(`Connecting... <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                                  <span class="sr-only">Loading...</span>`);
  try{
    let currentUser = await Parse.Web3.authenticate();
    if(currentUser){
      location.reload();
    }
  } catch (error) {
...
async function getOnSaleDetails(){
  try {
    let offerDetails = await Parse.Cloud.run("getOfferDetails");
    ...
    for (i = 0; i < offerDetailsLength; i++) {
      console.log("sdfsfdsfpakkk",);
      if(offerDetailsDuplicatesRemoved[i].active == true && offerDetailsDuplicatesRemoved[i].isSold == "false"){
        console.log("pakpaksdfsfdsfpakkk",);
...
async function getCurrentOwner(){
  try {
    let owner = await Parse.Cloud.run('getCurrentOwner');
...
async function getActiveArtworkInfo(){
  try {
    ...
    console.log("price Owner", price ); 

    let createdAt = await getTimeCreated();

    console.log("price CreatedAt", price ); 

    let activeArtwork = await Parse.Cloud.run('getArtwork');
    ...
    for (i = 0; i < activeArtwork.length; i++) {
      if(activeArtwork[i].active == true){
        ...
        let benefit = activeArtwork[i].benefit;
        let dectitle = activeArtwork[i].dectitle;
console.log("path check",path);
        ...
        if(tokenAddress.toLowerCase() + id.toLowerCase() == token.toLowerCase()){
          ...
          $('#dectitle' + tokenAddress + id).html(dectitle);
          ...
          if(benefit == ''){
            ...
            $('#benefit' + tokenAddress + id).css({'color': "#888", 'font-size': ".8rem", 'font-weight': '500'});
            $('#benefit' + tokenAddress + id).html('(No benefit given)');
          } else{
            $('#benefit' + tokenAddress + id).html(benefit);
          }




          if(picReader.length == 0 ){
           
            $('.vieimage').css('display', 'none');      
            console.log("sdfsfsfds23423dfg") ;     
          } 

         if(picReader.length != 0 )
          {
            $('.vieimage1').css('display', 'none');  
          }
...
async function getInactiveArtworkInfo(){
  try {
    ...
    let inactiveArtwork = await Parse.Cloud.run('getArtwork');
    ...
    for (i = 0; i < inactiveArtwork.length; i++) {
      if(inactiveArtwork[i].active == false){
        ...
        let dectitle = inactiveArtwork[i].dectitle;
        let benefit = inactiveArtwork[i].benefit;
...
async function getOwnerPhoto(tokenAddress, id, owner){
  ...
  try{
    let users = await Parse.Cloud.run('getAllUsers');
...
async function showBellsFilled(tokenAddress, id){
  if(user){
    ...
    let likeQuery = await Parse.Cloud.run('userEncouragedThisArtwork', params);
    if(likeQuery){
...
function encourageButton(tokenAddress, id){
  $('#encourageBell' + tokenAddress + id).click(async ()=>{
    if(user){
      ...
      let encourage = await Parse.Cloud.run('encourage', params);
      ...
      let encourageQuery = await Parse.Cloud.run('userEncouragedThisArtwork', params);
      if(encourageQuery){
        $('#encourageToSellBtn' + tokenAddress + id).removeClass('btn-secondary');
        $('#encourageToSellBtn' + tokenAddress + id).addClass('btn-warning');
        $('#encourageToSellBtn' + tokenAddress + id).html('Encouraged To Sell ✅');
...
async function showHeartsFilled(tokenAddress, id){
  if(user){
    ...
    let likeQuery = await Parse.Cloud.run('userLikesThisArtwork', params);
    if(likeQuery){
...
function likeButton(tokenAddress, id, likes){
  ...
  $('#like' + tokenAddress + id).click(async ()=>{
    if(user){
      ...
      let like = await Parse.Cloud.run('like', params);
      if(like || !like){
...
async function saveNewInfoCloudFunction(tokenAddress, id){
  ...
  const update = await Parse.Cloud.run('updateAdditionalInfo', params);
};
...
function shareOptions(tokenAddress, id){
  //obviously changing 192.168.10.87.sslip.io to real url when hosted
...
async function ifOwnerNotInDatabase(tokenAddress, id, owner){
  ...
  let isAddressIn = await Parse.Cloud.run('isAddressInDatabase', params);
  if(!isAddressIn){
  ...
  $('#unlockableContentBtn' + tokenAddress + id).click(async()=>{
    ...
    try{
      if(user.attributes.ethAddress.toLowerCase() == owner.toLowerCase()){
        ...
        const content = await Parse.Cloud.run('getUnlockableContent', params);
        console.log(content);
  ...
  let nftCard = `<!-- image -->
                ...
                <div class="info-wrapper col-xs-12 col-sm-12 col-md-12 col-lg-4">
                  <div class="info-section">
                    <div class="top-row-token-page">
                      <div class="owner-div row">
                        <div class="owner-photo">
                          <a class="anchor" id='owner`+tokenAddress+id+`' href="http://192.168.100.161/profile.html?address=`+owner+`"><img loading="lazy" class="owner shadow-sm" id="ownerPhoto`+tokenAddress+id+`" src="" width="40" alt="creator photo">
                        ...
                        <div class="name">
                          <a class="anchor" id="ownerNameAnchor`+tokenAddress+id+`" href="http://192.168.100.161/profile.html?address=`+owner+`">
                    ...
                    <div class="dectitle">
                    <span id="dectitle`+tokenAddress+id+`"></span>
                  </div>
                  ...
                  <div class="benefit">
                  <div class="text-white" id="benefit`+tokenAddress+id+`"></div>                 
                  </div>
                  ...
                  <span > Description :<\span>
                    ...
                    <div class="text-white" id="description`+tokenAddress+id+`"></div>                 
                    </div>
                    ...
                    <a class="btn btn-primary  id="create" href="http://192.168.100.161/tokenn.html?token=`+tokenAddress+id+`">View Image</a>
                  ...
                  <span style="color: rgb(104 234 255 / 90%);"> Category :<\span> &nbsp;&nbsp; &nbsp;&nbsp; <span class="text-white" id="category`+tokenAddress+id+`"></span>                   
                   
                    </div>
                    <div class="additional-info">
                      <a class="text-light mb-3 additional-info-tag" id="additionalInfo`+tokenAddress+id+`"></a>
                    </div>
                    ...
                    <div class="creator-div row">
                      <div class="creator-photo">
                        <a class="anchor" href="http://192.168.100.161/profile.html?address=`+creator+`"><img loading="lazy" class="creator shadow-sm" id="creatorPhoto`+tokenAddress+id+`" src="" width="40" alt="creator photo">
                      ...
                      <div class="name">
                        <a class="anchor" href="http://192.168.100.161/profile.html?address=`+creator+`">
