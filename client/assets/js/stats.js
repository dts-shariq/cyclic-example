...
const ETH_USD_PRICE_UR = "/simple/price?ids=ethereum&vs_currencies=usd";
...
async function getBnbPrice(){
  let bnbPrice = BASE_URL + ETH_USD_PRICE_UR;
  const response = await fetch(bnbPrice);
  const data = await response.json();
...
async function getProfits(){
  ...
  if(totalProfit == undefined){
    ...
    $('#profitsFromSelling').html(`${profitInBnb} ETH`);
...
async function getRoyaltiesReceived(){
  ...
  if(totalRoyalties == undefined){
    ...
    $('#royaltiesReceived').html(`${royaltiesInBnb} ETH`);
...
async function getTipsReceived(){
  ...
  if(totalTips == undefined){
    ...
    $('#tipsReceived').html(`${tipsInBnb} ETH`);
