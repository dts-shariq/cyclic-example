...
async function allCount(){
  ...
  console.log("hbsfabdj rtest ", activeCount);
...
async function forSaleCount(){
  ...
  console.log("hbsfabdj rtes t 222 ", activeCount);
...
async function notForSaleCount(){
  let inactiveArtwork = await Parse.Cloud.run('getArtwork');
  let inactiveCount = inactiveArtwork.filter(item => !item.active).length;
  console.log("hbsfabdj rtes t 222222 ", inactiveCount);
...
$('#connectWalletModalBtn').click(async () =>{
  ...
  $('#connectWalletBtn').html(`Connecting... <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                                  <span class="sr-only">Loading...</span>`);
  try{
    let currentUser =  handleAuth('Metamask');
    if(currentUser){
      location.reload();
    }
  } catch (error) {
...
async function recentlySold(){
  try {
    let soldDetails = await Parse.Cloud.run("getSoldDetails");
    let getArtwork_path = await Parse.Cloud.run('getArtwork');//Change File Size Show
    ...
    for (i = 0; i < restrictedToRecentEightSalesArray.length; i++) {
     ...
     // let cover = getArtwork_path[i].path;
...
function putOnSale(tokenAddress, id, royalty, creator){
  $('#putOnSaleBtn' + tokenAddress + id).click(async()=>{
	...
	//	listing=listing.toString();
	 ...
	 await OpenMintMarketplaceInstance.methods.setOffer(amountInWei, id, tokenAddress).send({from: account});	  
