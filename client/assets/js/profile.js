...
const paymentGatewayAddress = "0x692413DA5Ba1a7B0890A008531442C14167eF496";
...
async function getForSaleCount(){
  let ifOfferDetails = await Parse.Cloud.run("getOfferDetails");
  let ifOfferDetailsDuplicatesRemoved = removeDuplicates(ifOfferDetails, it => it.tokenId);
  const count = ifOfferDetailsDuplicatesRemoved.filter(item => item.owner.toLowerCase() == address.toLowerCase() && !(item.isSold=='True') && item.active).length;
  $('#forSaleCount').html(count);
  return count;
};

async function getOwnsCount(){
  let artwork = await Parse.Cloud.run('getArtwork');
  console.log(1);
  ...
  if(count == 0){
    console.log(3);
    $('.cardDivs').html(`<div class="list-group"><span class="sub-text mt-5">No artwork currently 1owned</span></div>`);
  }
  return count;
};
// async function getcategoryCount(){
//   let artwork = await Parse.Cloud.run('category');
//   const count = artwork.filter(item => item.currentOwner.toLowerCase() == address.toLowerCase()).length;
...
//     $('.cardDivs').html(`<div class="list-group"><span class="sub-text mt-5">No artwork currently 2owned</span></div>`);
//   }
//   return count;
// };
...
async function getFollowingCount(){
  const params = {ethAddress: address}
  let following = await Parse.Cloud.run('getFollowing', params);
...
async function getFollowerCount(){
  const params = {ethAddress: address}
  let followers = await Parse.Cloud.run('getFollowers', params);
...
async function doesUserFollow(){
  const params = {ethAddress: address};
  let doesFollow = await Parse.Cloud.run('doesUserFollow', params);
...
$("#followButton").click(async() =>{
  ...
  let follow = await Parse.Cloud.run('follow', params);
  if(follow || !follow){
    $('#followButton').prop('disabled', false);
  }
  let followers = await Parse.Cloud.run('followers', params);
  doesUserFollow();
});
...
$('#tipModal').on('hidden.bs.modal', function (e) {
  ...
  $('#tipAmountInEth').html('0 ILIFETOKEN');
...
async function setTip(toAddress, tipInWei){
  ...
  await Parse.Cloud.run('setTotalTips', params);
};
...
async function withdrawBtn(){
  if(!user || user.attributes.ethAddress.toLowerCase() !== address.toLowerCase()){
    ...
    if(profit > 0){
      ...
      $('#amountToWithdraw').html(` ${profit} ILIFETOKEN`);
    } else{
...
$("#withdraw").click(async function(){
  ...
  let profit = await getMyBalance();
  $('#withdrawText').html(`Confirm to withdraw your profits of <span class="sale-profit">${profit} ILIFETOKEN</span>`);
});
...
async function withdrawProfits(){
  ...
  try {
    ...
    $('#withdrawText').html(`Confirm to withdraw your profits of <span class="sale-profit">0 ILIFETOKEN</span>`);
...
$('#owns').click(async()=>{
  ...
  if(count == 0){
    $('.cardDivs').html(`<div class="list-group"><span class="sub-text mt-5">No artwork currently 3owned</span></div>`);
  } else {
    getActiveOwnedArt();
    getInactiveOwnedArt();
  }
...
$('#category').click(async()=>{
  ...
  if(count == 0){
    $('.cardDivs').html(`<div class="list-group"><span class="sub-text mt-5">No artwork currently 4owned</span></div>`);
  } else {
...
async function getFollowers(){
  ...
  if(followers == undefined || followers.length == 0){
    ...
    for (i = 0; i < followers.length; i++) {
      ...
      let userInfo = await Parse.Cloud.run('getUser', params);
...
async function getActiveCategory(){
  try{
    let ifOfferDetails = await Parse.Cloud.run("category");
    let ifOfferDetailsLength = removeDuplicates(ifOfferDetails, it => it.tokenId).length;
    let ifOfferDetailsDuplicatesRemoved = removeDuplicates(ifOfferDetails, it => it.tokenId);
...
async function getInactivecategory(){
  try{
    let artwork = await Parse.Cloud.run('category');
    for (i = 0; i < artwork.length; i++) {
      if(artwork[i].currentOwner.toLowerCase() == address.toLowerCase() && artwork[i].active == false){
...
async function getProfileDetails(){
  try{
    let userDetails = await Parse.Cloud.run('getAllUsers');
    for (i = 0; i < userDetails.length; i++) {
      const str = userDetails[i].ethAddress || '';
      const stra =address|| '';
     if(str.toLowerCase() == stra.toLowerCase()){
                    ...
                    <div id="card`+tokenAddress+id+`" class="card shadow-sm">
                      <div class="top-row">
                        <div class="creator-photo">
                          <a id='owner`+tokenAddress+id+`' href="http://192.168.100.161/profile.html?address=`+owner+`"><img class="owner shadow-sm" id="ownerPhoto`+tokenAddress+id+`" src="" width="40" alt="owner photo">
...
function userCard(ethAddress){
  let userCard = `<li id="userItem`+ethAddress+`" class="list-group-item d-flex justify-content-between align-items-center">
                    <a href="http://192.168.100.161/profile.html?address=`+ethAddress+`">
                      ...
                      <form>
                        <div id="changePriceInputGroup`+tokenAddress+id+`" class="price-input-group">
                          <div class="input-group">
                            <input id="changePriceInput`+tokenAddress+id+`" type="text" class="form-control input-styling" placeholder="Enter ILIFETOKEN" aria-label="ether amount">
                          </div>
                            ...
                            <div class="modal-body">
                              ...
                              <form>
                                <div id="priceInputGroup`+tokenAddress+id+`" class="price-input-group">
                                  <div class="input-group">
                                    <input id="salePriceInput`+tokenAddress+id+`" type="text" class="form-control input-styling" placeholder="Enter ILIFETOKEN" aria-label="ether amount">
                                  </div>
