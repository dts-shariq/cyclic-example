...
const catReal="Real Estate";
//const catMedical="Health Care";
...
async function nftCount(cleanedQuery){
  ...
  let activeArtworkQuery = ifOfferDetailsDuplicatesRemoved.filter(item => !(item.isSold=='True') && item.active && item.name.toLowerCase().includes(cleanedQuery) ||
                                                                  !(item.isSold=='True') && item.active && item.category.toLowerCase().includes(cleanedQuery) ||
                                                                  !(item.isSold=='True') && item.active && item.creator.toLowerCase().includes(cleanedQuery)||
                                                                  !(item.isSold=='True') && item.active && item.owner.toLowerCase().includes(cleanedQuery)||
                                                                  !(item.isSold=='True') && item.active && item.creator.toLowerCase().includes(addressFromUsername)||
                                                                  !(item.isSold=='True') && item.active && item.owner.toLowerCase().includes(addressFromUsername));
  let inactiveArtwork = await Moralis.Cloud.run('getArtwork');
  let inactiveArtworkQuery = inactiveArtwork.filter(item => !item.active && item.name.toLowerCase().includes(cleanedQuery) ||
                                                    !item.active && item.category.toLowerCase().includes(cleanedQuery)||
  ...
  try{
    ...
    for (i = 0; i < activeArtworkQuery.length; i++) {
      if(activeArtworkQuery[i].active == true && activeArtworkQuery[i].isSold == "false"){
        ...
        let category = activeArtworkQuery[i].category;
     //   if(category==catReal || category==catMedical){
          if(category==catReal ){
        }
        else{
        $('#loader').css('display', 'none');
  ...
  try{
    ...
    for (i = 0; i < inactiveArtworkQuery.length; i++) {
      if(inactiveArtworkQuery[i].active == false){
        ...
        let category = inactiveArtworkQuery[i].category;
       // if(category==catReal || category==catMedical){
          if(category==catReal ){
        ...
        $('#category' + tokenAddress + id).html(name);
        $('#name' + tokenAddress + id).html(name);
                      ...
                      <form>
                        <div id="changePriceInputGroup`+tokenAddress+id+`" class="price-input-group">
                          <div class="input-group">
                            <input id="changePriceInput`+tokenAddress+id+`" type="text" class="form-control input-styling" placeholder="Enter price inILIFETOKEN" aria-label="ether amount">
                          </div>
                          ...
                          <div class="price-calculator price-info">
                            ...
                            <span>Your profit will be: <span id="changePriceSaleProfit`+tokenAddress+id+`" class="sale-profit">0ILIFETOKEN</span> <span id="changePriceUSDProfit`+tokenAddress+id+`">$0.00</span></span>
                          </div>
                        </div>
                      </form>
                            ...
                            <div class="modal-body">
                              ...
                              <form>
                                <div id="priceInputGroup`+tokenAddress+id+`" class="price-input-group">
                                  <div class="input-group">
                                    <input id="salePriceInput`+tokenAddress+id+`" type="text" class="form-control input-styling" placeholder="Enter price inILIFETOKEN" aria-label="ether amount">
                                  </div>

                                  <div class="price-calculator price-info">
                                    <span>Service Fee Upon Sale <span>2%</span></span><br>
                                    <span id="ifOwnerNotCreator`+tokenAddress+id+`">Creator's Royalty <span id="royalty`+tokenAddress+id+`"></span><br></span>
                                    <span>Your profit will be: <span id="saleProfit`+tokenAddress+id+`" class="sale-profit">0ILIFETOKEN</span> <span id="usdProfit`+tokenAddress+id+`">$0.00</span></span>
                                  </div>
                                </div>
                              </form>
...
function userDiv(ethAddress){
  let userCard = `<div id="userCard`+ethAddress+`" class="grid-helper col-xs-12 col-sm-6 col-md-4 col-lg-4 col-xl-3">
                    <a id="userCardAnchor`+ethAddress+`" href="http://192.168.100.161/profile.html?address=`+ethAddress+`">
